...
df4.columns = ['description']
df4['description'] = 'warting'  #df4['description'].apply( lambda x:str(x).replace(',',' ').replace('，',' '))
...
df['ctrip_rank'] = 'waiting'
df['local_rank'] = 'waiting'
# df['comfort'] = 'waiting'
df['eat_json'] = 'waiting'
...
df['tag_json'] = 'waiting'
df['age_desc_json'] = 'waiting'
df['sex_desc_json'] = 'waiting'
df['area_desc_json'] = 'waiting'
df['cost_desc_json'] = 'waiting'


df = df[[ 'id','name','description','figure_url','video_url'
...
df.to_csv('../../data/scenic.csv',encoding='utf8',index=False)
# print(df.head())
...
CREATE TABLE IF NOT EXISTS `mydb`.`scenic` (
  ...
  `area_desc_json` VARCHAR(180) NULL,
